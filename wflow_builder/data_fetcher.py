...
class DataFetcher:
    ...
    def fetch_era5_data(self, start_date, end_date, buffer_degrees=2.0):
        """
        ...
        Args:
            ...
            buffer_degrees: Buffer to add around the extent (default: 1.0 degree)
        ...
        try:
            # Get original extent from config
            original_extent = self.config.extent
            min_lon, min_lat, max_lon, max_lat = original_extent
            
            # Expand extent by buffer_degrees in all directions
            expanded_extent = [
                min_lon - buffer_degrees,  # Add buffer to west
                min_lat - buffer_degrees,  # Add buffer to south
                max_lon + buffer_degrees,  # Add buffer to east
                max_lat + buffer_degrees   # Add buffer to north
            ]
            
            logger.info(f"Original extent: {original_extent}")
            logger.info(f"Expanded extent for ERA5 (buffer={buffer_degrees}°): {expanded_extent}")
            
            # Create region geometry with expanded extent
            region_geom = ee.Geometry.Rectangle(expanded_extent)
